...
int tryAll (int step) {
    if (step+lowerBound(N) > upperBound )  return 0;

    if (giaRisolto()) {
      //cout << "Solution found ! " << step << endl;
       upperBound = step;
    ...
    for (int i = 1; i < L; i++) {
        if( (i != pos_XX) && (i != pos_XX +1 ) && (i != pos_XX -1) ) {
              moves[step] = i;
	      spostamento(i);
              int add = tryAll (step +1);
              if(step==0) {
                 if(add>0) {
		   cout << "num soluzioni: " << add << " beginning with:" << endl;
		   stampaTreno();                  
                 }
              }
	      num_sol += add;
...
int main (void) {
	...
	initTreno();
	cout << "Numero soluzioni: " << tryAll(0) << endl;
	stampaBestSol ();
	assert(verifyBestSol());
